...
if (-not $officeExpectedVersion) {
  ...
  [void]$report.AppendLine('')
  [void]$report.AppendLine('Ofis hedef sürüm kaydı bulunamadı. Aktif kök yerel klasör olmalıdır (pCloud yalnızca manuel yedek/arşiv). Örnek: `npm run live:version-check -- -RootPath "D:\BARAN_GLOBAL_EKSPERTIZ\2026" -ExpectedVersion 0.5.0 -SetExpected -RegisterThisPC` komutu çalıştırılmalıdır.')
  [void]$report.AppendLine('')
