...
assert(pkg.version === '0.6.10', 'Paket sürümü v0.6.10 olarak sabitlendi', `version=${pkg.version}`);
...
assert(knowledgeSource.includes('chunkCount?: number') && knowledgeSource.includes('chunkCountForSource') && knowledgePanelComponentSource.includes('Parça sayısı'), 'v0.6.0 P2-B kaynak/chunk sayisi read-only durumda gorunur', 'Bilgi Bankasi chunk sayisi guard eksik');
assert(knowledgePanelComponentSource.includes('Yalnız yerel / ücretsiz / salt okunur') && knowledgePanelComponentSource.includes('Ücretli servis') && knowledgePanelComponentSource.includes('Harici bağlantı') && knowledgePanelComponentSource.includes('Yazma modu') && knowledgePanelComponentSource.includes('takip.json, Excel veya dosya klasörlerine yazma yapmaz'), 'v0.6.0 P2-B panel yalniz-yerel/ucretsiz/salt okunur guvenlik ozetini gosterir', 'Bilgi Bankasi guvenlik ozeti eksik');
...
assert(rendererSource.includes('loadKnowledgeImportDryRunPlan') && rendererSource.includes('window.hasarbotu.dryRunKnowledgeImportPlan') && knowledgePanelComponentSource.includes('knowledgeImportDryRunPlan') && knowledgePanelComponentSource.includes('Canlı Deneme Planı'), 'v0.6.0 P3-H panel read-only dry-run IPC sonucunu canli ve read-only gosterir', 'P3-H canli IPC paneli baglanmadi');
...
assert(knowledgePanelComponentSource.includes('buildKnowledgeImportCommitPlan') && knowledgePanelComponentSource.includes('Kalıcı Kayıt Ön İzleme') && knowledgePanelComponentSource.includes('data-action="knowledge-commit-text-preview"') && !/data-action="[^"]*(knowledge-import|knowledge-write|knowledge-save|knowledge-apply|knowledge-delete|knowledge-export)/i.test(knowledgePanelComponentSource), 'v0.6.0 P4-E2-B panel commit on izlemeyi gosterir ve yalniz tek narrow commit aksiyonunu tasir', 'P4-E2-B commit on izleme paneli eksik veya yasak aksiyon var');
...
assert(p3KnowledgeImportDisplaySource.includes('buildKnowledgeImportPlanViewModel') && p3KnowledgeImportDisplaySource.includes('renderKnowledgeImportPlanView') && p3KnowledgeImportDisplaySource.includes('İçe Aktarma Plan Hazırlığı') && p3KnowledgeImportDisplaySource.includes('canWrite'), 'v0.6.0 P3-B dry-run import plan view model ve pasif component mevcut', 'P3-B view/model eksik');
assert(p3KnowledgeImportDisplaySource.includes('Reddedildi') && p3KnowledgeImportDisplaySource.includes('Sadece plan') && p3KnowledgeImportDisplaySource.includes('Kullanici onayi gerekir') && p3KnowledgeImportDisplaySource.includes('Gelecek içe aktarma için uygun'), 'v0.6.0 P3-B candidate permission durumlari goruntuleme modelinde ayriliyor', 'P3-B permission label eksik');
assert(p3KnowledgeImportDisplaySource.includes('dosya icerigi okunmaz') && p3KnowledgeImportDisplaySource.includes('kalici kaynak eklenmez') && p3KnowledgeImportDisplaySource.includes('takip.json, Excel veya AppData yazilmaz') && p3KnowledgeImportDisplaySource.includes('içe aktarma çalıştırılmaz'), 'v0.6.0 P3-B pasif goruntuleme guvenlik metinlerini tasir', 'P3-B guvenlik metni eksik');
...
// --- v0.6.0 P4-E3: User Knowledge Store'u Bilgi Bankasi aramasina read-only dahil etme guardlari ---
const userKnowledgeSearchServiceSource = await fs.readFile('src/main/services/knowledge/user-knowledge-search-service.ts', 'utf-8');
assert(userKnowledgeSearchServiceSource.includes('searchUserKnowledgeEntries') && userKnowledgeSearchServiceSource.includes('mergeUserKnowledgeIntoResponse') && userKnowledgeSearchServiceSource.includes("origin: 'user'") && userKnowledgeSearchServiceSource.includes('Kullanıcı Kaynağı'), 'v0.6.0 P4-E3 user store arama servisi kullanici sonucunu Kullanici Kaynagi origin=user olarak uretir', 'P4-E3 user arama servisi eksik');
assert(!/\.write\(|UserKnowledgeStoreFile|from ['"]node:fs|from ['"]fs|\bfs\.|atomicWrite|writeFile|filePath|pdf2json|tesseract|mammoth|OpenAI|Claude|Gemini|fetch\(|axios|sqlite|embedding/i.test(userKnowledgeSearchServiceSource), 'v0.6.0 P4-E3 user store arama servisi saf/read-only: store-write/fs/filePath/parser/provider izi yok', 'P4-E3 arama servisi yasak iz tasiyor');
const mainIpcOnlySource = await fs.readFile('src/main/ipc.ts', 'utf-8');
const knowledgeSearchHandlerSlice = mainIpcOnlySource.slice(mainIpcOnlySource.indexOf('private async searchKnowledgeWithUserStore'), mainIpcOnlySource.indexOf('private async safe<'));
assert(knowledgeSearchHandlerSlice.length > 0 && knowledgeSearchHandlerSlice.includes('new UserKnowledgeStoreFile(this.cache.cacheRoot).read()') && knowledgeSearchHandlerSlice.includes('searchUserKnowledgeEntries') && knowledgeSearchHandlerSlice.includes('mergeUserKnowledgeIntoResponse') && !knowledgeSearchHandlerSlice.includes('.write('), 'v0.6.0 P4-E3 knowledge search handler user store yalniz READ ile dahil eder; bu yolda write yok', 'P4-E3 search handler read-only entegrasyonu eksik veya write iz var');
assert(knowledgeSource.includes("origin?: 'seed' | 'user'") && knowledgeSource.includes('sourceLabel?: string'), 'v0.6.0 P4-E3 KnowledgeSearchResult tipinde origin/sourceLabel ayrim alanlari minimal eklendi', 'P4-E3 result origin/sourceLabel alani eksik');
assert(knowledgePanelComponentSource.includes("origin === 'user'") && knowledgePanelComponentSource.includes('Kullanıcı Kaynağı') && knowledgePanelComponentSource.includes('Yerel kullanıcı bilgi deposu') && !knowledgePanelComponentSource.includes('filePath') && !/data-action="[^"]*(delete|edit|reimport|open-file|reveal|remove)[^"]*"/i.test(knowledgePanelComponentSource), 'v0.6.0 P4-E3 panel kullanici sonucunu read-only etiketler; sil/duzenle/yeniden-import/dosya-ac aksiyonu ve filePath yok', 'P4-E3 panel etiket/aksiyon/filePath guard ihlali');
const userStoreModuleSource = await fs.readFile('src/main/local-cache/user-knowledge-store.ts', 'utf-8');
const commitServiceWriteSource = await fs.readFile('src/main/services/knowledge/knowledge-import-commit-service.ts', 'utf-8');
assert(userStoreModuleSource.includes('async write(') && /store\.write\(|\.write\(nextStore\)/.test(commitServiceWriteSource) && !userKnowledgeSearchServiceSource.includes('.write(') && !knowledgeSearchHandlerSlice.includes('.write('), 'v0.6.0 P4-E3 UserKnowledgeStore.write yalniz store modulu + dar commit servisinde; arama yolu yazmaz', 'P4-E3 store write yeni yerde kullanilmis');

// --- v0.6.0 P4-E4: Bilgi Bankasi kaynak filtresi + bos depo durum mesaji guardlari ---
const knowledgeSourceFilterSource = await fs.readFile('src/shared/knowledge/knowledge-source-filter.ts', 'utf-8');
assert(knowledgeSourceFilterSource.includes('filterKnowledgeResultsByOrigin') && knowledgeSourceFilterSource.includes('isKnowledgeSourceFilter') && knowledgeSourceFilterSource.includes("origin === 'user'") && !/\.write\(|UserKnowledgeStoreFile|from ['"]node:fs|from ['"]fs|\bfs\.|filePath|fetch\(|axios|OpenAI|Claude|Gemini/i.test(knowledgeSourceFilterSource), 'v0.6.0 P4-E4 kaynak filtre modulu saf read-only origin suzme yapar; yazma/fs/filePath/provider yok', 'P4-E4 filtre modulu eksik veya yasak iz tasiyor');
assert(knowledgeSource.includes('UserKnowledgeStoreSearchStatus') && knowledgeSource.includes('userStoreStatus?'), 'v0.6.0 P4-E4 KnowledgeSearchResponse read-only userStoreStatus metadata tasir', 'P4-E4 userStoreStatus tipi eksik');
assert(knowledgeSearchHandlerSlice.includes('entryCount') && knowledgeSearchHandlerSlice.includes('matchedCount') && knowledgeSearchHandlerSlice.includes('available') && !knowledgeSearchHandlerSlice.includes('.write('), 'v0.6.0 P4-E4 search handler userStoreStatus (entryCount/matchedCount/available) read-only uretir', 'P4-E4 search handler userStoreStatus uretmiyor');
const knowledgeStateComponentSource = await fs.readFile('src/renderer/app/state.ts', 'utf-8');
assert(knowledgeStateComponentSource.includes("knowledgeSourceFilter: 'all'") && knowledgeStateComponentSource.includes('knowledgeSourceFilter: KnowledgeSourceFilter'), 'v0.6.0 P4-E4 renderer state knowledgeSourceFilter all baslangicli tanimli', 'P4-E4 state filtre alani eksik');
assert(rendererSource.includes('setKnowledgeSourceFilter') && rendererSource.includes("case 'knowledge-source-filter'"), 'v0.6.0 P4-E4 renderer kaynak filtre aksiyonu bagli', 'P4-E4 renderer filtre aksiyonu eksik');
assert(knowledgePanelComponentSource.includes('renderKnowledgeSourceFilter') && knowledgePanelComponentSource.includes('data-action="knowledge-source-filter"') && knowledgePanelComponentSource.includes('Yerleşik') && knowledgePanelComponentSource.includes('Kullanıcı bilgi deposunda henüz kayıt yok.') && knowledgePanelComponentSource.includes('Kullanıcı kaynaklarında eşleşen kayıt bulunamadı.') && knowledgePanelComponentSource.includes('Kullanıcı bilgi deposu okunamadı; yerleşik kaynaklar gösteriliyor.') && !knowledgePanelComponentSource.includes('filePath'), 'v0.6.0 P4-E4 panel kaynak filtresi + bos/eslesme-yok/okuma-hatasi mesajlari icerir; filePath yok', 'P4-E4 panel filtre/mesaj guard ihlali');
...
const knowledgeImportPlanViewComponentSource = await fs.readFile('src/renderer/app/components/knowledge-import-plan-view.ts', 'utf-8');
const knowledgeUiDisplaySource = `${stripCodeComments(knowledgePanelComponentSource)}\n${stripCodeComments(knowledgeImportPlanViewComponentSource)}`;
...
assert(knowledgePanelComponentSource.includes('Canlı Deneme Planı') && knowledgePanelComponentSource.includes('Kalıcı Kayıt Ön İzleme') && knowledgePanelComponentSource.includes('salt okunur') && knowledgePanelComponentSource.includes('Yalnız yerel') && knowledgePanelComponentSource.includes('Kalıcı olarak ekle'), 'v0.6.0 UI Bilgi Bankasi Turkce karsiliklar (Deneme Plani/Kalici Kayit/salt okunur) mevcut', 'Bilgi Bankasi Turkce karsiliklari eksik');
const settingsLearningSource = await fs.readFile('src/renderer/app/components/settings.ts', 'utf-8');
assert(knowledgeStateComponentSource.includes('laborLearningExpanded: Record<string, boolean>') && knowledgeStateComponentSource.includes('laborLearningExpanded: {}'), 'v0.6.0 UI sozluk ac/kapat durumu bellek-ici state olarak tanimli (varsayilan bos)', 'laborLearningExpanded state eksik');
assert(settingsLearningSource.includes('labor-learning-row compact') && settingsLearningSource.includes('data-action="labor-learning-toggle"') && settingsLearningSource.includes('labor-learning-detail') && settingsLearningSource.includes("'Kapat' : 'Aç'") && settingsLearningSource.includes('data-action="labor-learning-update"') && settingsLearningSource.includes('data-action="labor-learning-delete"'), 'v0.6.0 UI AI iscilik ogrenme sozlugu kompakt akordeon (varsayilan kapali) + kaydet/sil korunur', 'sozluk kompakt akordeon yapisi eksik');
assert(rendererSource.includes('toggleLaborLearningExpanded') && rendererSource.includes("case 'labor-learning-toggle'"), 'v0.6.0 UI sozluk ac/kapat aksiyonu yalniz UI bellegini gunceller (yeni IPC/yazma yok)', 'sozluk toggle aksiyonu eksik');

// --- v0.6.1: Gemini AI parca-okuma gecici hata (HTTP 503/timeout/network) yonetimi guardlari ---
const geminiClientComponentSource = await fs.readFile('src/main/import/gemini-client.ts', 'utf-8');
assert(geminiClientComponentSource.includes('createTransientAiError') && /status >= 500[\s\S]*?createTransientAiError/.test(geminiClientComponentSource) && geminiClientComponentSource.includes("error.name === 'AbortError'") && geminiClientComponentSource.includes('instanceof TypeError'), 'v0.6.1 gemini-client 5xx/timeout/network hatalarini gecici hata kodu ile firlatir', 'gemini-client gecici hata siniflandirmasi eksik');
const aiTransientErrorSource = await fs.readFile('src/shared/ai/ai-transient-error.ts', 'utf-8');
assert(aiTransientErrorSource.includes("AI_TRANSIENT_ERROR_CODE = 'AI_SERVICE_TRANSIENT'") && aiTransientErrorSource.includes('AI servisi geçici olarak cevap vermiyor') && aiTransientErrorSource.includes('manuel') && !/\bfetch\(|axios|\.write\(|takip\.json|OpenAI|Claude/i.test(aiTransientErrorSource), 'v0.6.1 gecici AI hata modulu saf: kod + Turkce kullanici mesaji; fetch/yazma/provider yok', 'v0.6.1 gecici AI hata modulu eksik veya yasak iz tasiyor');
const partsActionOfficeSlice = rendererSource.slice(rendererSource.indexOf('async function analyzePartsPhotoAction'), rendererSource.indexOf('async function loadPartsUserTerms'));
